...
describe("Test SBART functions",{
    ...
    source("output/KIM/output_sbart_full.R")
    ...
    it("Should return Kim's results",{
        expect_equal(model$covariates_selection_chain, output_sbart_full$covariates_selection_chain)
        expect_equal(model$spatial_theta_chain, output_sbart_full$spatial_theta_chain)
        expect_equal(model$sigma2_chain, output_sbart_full$sigma2_chain)
        expect_equal(model$trees_chain, output_sbart_full$trees_chain)
        expect_equal(model$w_selection_chain, output_sbart_full$w_selection_chain)
    })
})