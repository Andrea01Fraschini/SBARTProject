...
x1 <- divide.ind*rnorm(K, 2, 1) + (1-divide.ind)*rnorm(K, -2, 1)
x2 <- rnorm(K, 1-0.5*(Grid[,2]-4), 0.1)
...
phi <- rep(0, K)
phi[which(divide.ind==1)] <- rmnorm(1, rep(0,K), 0.25*exp(-0.05 * Distance))[which(divide.ind==1)]
phi[which(divide.ind==0)] <- rmnorm(1, rep(0,K), 0.5*exp(-0.1 * Distance))[which(divide.ind==0)]
...
missing <- mis.ind <- sample(1:64, 39, replace=F)
Y[missing] <- NA
...
m <- 30                  # Num. of Trees: default setting 100
...
# Weight matrix
a.weight <- 2 # initial value for a_w parameter in the weight
...
W.wind <- (W1^(I(a.weight==1)))*(W2^(I(a.weight==2)))*(W3^(I(a.weight==3)))*(W4^(I(a.weight==4)))*(W5^(I(a.weight==5)))
...
W.wind.full <- (W1^(I(a.weight==1)))*(W2^(I(a.weight==2)))*(W3^(I(a.weight==3)))*(W4^(I(a.weight==4)))*(W5^(I(a.weight==5)))
...
n.iter <- 10000            # Num. of Iterations
...
save(Y.true, Y, shift,Y.da, mis.ind, A.WEIGHT, ind, Sigma2, file=paste0("Sim2_W_",process,".RData"))
