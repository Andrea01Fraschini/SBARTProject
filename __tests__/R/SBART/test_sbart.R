...
describe("Test SBART functions",{
    ...
    source("output/KIM/output_sbart.R")
    ...
    it("Should return Kim's results",{
        expect_equal(model$covariates_selection_chain, output_sbart$covariates_selection_chain)
        expect_equal(model$spatial_theta_chain, output_sbart$spatial_theta_chain)
        expect_equal(model$sigma2_chain, output_sbart$sigma2_chain)
        expect_equal(model$trees_chain, output_sbart$trees_chain)
        expect_equal(model$w_selection_chain, output_sbart$w_selection_chain)
    })
})